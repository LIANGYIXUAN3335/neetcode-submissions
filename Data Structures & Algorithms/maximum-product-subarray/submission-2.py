class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        numMax ,numMin,res = nums[0],nums[0] ,nums[0]
        for i in range(1,n):
            temp = max(nums[i],numMax * nums[i],numMin * nums[i])
            numMin = min(nums[i],numMax * nums[i],numMin * nums[i])
            numMax = temp
            res = max(res,numMax,numMin)
        return res
